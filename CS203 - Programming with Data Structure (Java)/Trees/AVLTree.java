...
public class AVLTree<E extends Comparable<E>> extends BST<E> {
	...
	public E find(int k, TreeNode node){
		if(node.left == null && k==1)
			return (E) node.element;
		else if(node.right == null && k==2)
			return (E) node.right.element;
		else if(k <= getSize(node.left)){
			...
			return find(k-getSize(node.left)-1,node.right);
		}
		return null;
	}
}