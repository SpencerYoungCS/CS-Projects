...
extends AbstractTree<E> implements Cloneable {
	...
	public void breadthFirstTraversal(TreeNode node){
		TreeNode<E> current = node; // Start from the root
		Queue<TreeNode> queue = new LinkedList<TreeNode>();
		if (current == null)
			return;
		queue.add(current);
		while (!queue.isEmpty()) {
			TreeNode checker = queue.peek();
			System.out.print(" " + queue.peek().element);
			queue.remove();
			if (checker.left != null)
				queue.add(checker.left);
			if (checker.right != null)
				queue.add(checker.right);
		}
		// implement the body
	...
	public int getNumberOfNonLeaves(TreeNode node){

	TreeNode<E> current =  new TreeNode<E>(null); // Start from the root
	current = node; // Start from the root
		if(current == null ){
			return 0;
		}
		if(current.left == null && current.right == null ) {
